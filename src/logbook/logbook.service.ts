...
export class LogbookService {
  ...
  //         // const LastAdditionalInformation = await this.logbooksRepository
  //         //   .findOne({
  ...
  async findLatest(): Promise<Logbook[]> {
    ...
    // const latestLogbookVw = await this.logbooksRepository.findOne({ vehicleTyp: 'VW' }).sort({ newMileAge: -1 }).limit(1).exec();
    // const latestLogbookFerrari = await this.logbooksRepository.findOne({ vehicleTyp: 'Ferrari' }).sort({ newMileAge: -1 }).limit(1).exec();
    // const latestLogbookPorsche = await this.logbooksRepository.findOne({ vehicleTyp: 'Porsche' }).sort({ newMileAge: -1 }).limit(1).exec();
    ...
    return this.logbooksRepository.findLastAddedLogbook();
  }
  //
  ...
  // async remove(_id: string): Promise<Logbook> {
  //   return await this.logbookModel.findByIdAndDelete(_id).exec();
  // }
  //
